...
# ── Android / Termux — native build (run this inside Termux on the device) ────
#
# Builds teamtype natively inside a Termux environment.  No NDK or
# cross-compilation is needed: Cargo targets the host system, which is already
# Android ARM64.
#
# Prerequisites (one-time setup, inside Termux):
#   pkg install rust binutils
#
# After building, run termux-elf-cleaner to fix the PT_TLS alignment and strip
# unsupported ELF sections that the Android linker warns about.  This is
# required for the binary to load correctly under Bionic.
#
# Verification:
#   file dist/teamtype-termux
#     → ELF 64-bit LSB pie executable (or shared object), ARM aarch64 ...
#   readelf -lW dist/teamtype-termux | grep TLS
#     → TLS ... 0x40     ← must be 0x40 (64), not 0x8

.PHONY: build-termux
build-termux: $(DIST_DIR)
	cargo build --release \
		--manifest-path daemon/Cargo.toml
	cp daemon/target/release/teamtype \
		$(DIST_DIR)/teamtype-termux
	termux-elf-cleaner $(DIST_DIR)/teamtype-termux
	@echo "Binary written to $(DIST_DIR)/teamtype-termux"

# ── Android / Termux (ARM64, cross-compiled from Linux with NDK) ───────────────
#
# Produces an ET_DYN PIE binary accepted by Android's Bionic linker.
# The aarch64-unknown-linux-musl target produces a statically linked ET_EXEC
# (e_type=2) which Bionic rejects; aarch64-linux-android produces ET_DYN
