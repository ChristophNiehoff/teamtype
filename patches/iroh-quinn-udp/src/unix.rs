        ...
        if n == -1 {
            ...
            match e.kind() {
                ...
                _ => {
                    ...
                    //
                    // Android kernels may claim to support GSO via setsockopt(UDP_SEGMENT) but
                    // then fail actual sendmsg calls with EIO. In that case we disable GSO and
                    // retry the current transmit without segmentation offload.
                    #[cfg(any(target_os = "linux", target_os = "android"))]
                    if let Some(libc::EIO) | Some(libc::EINVAL) = e.raw_os_error() {
                        ...
                        // Retry the current transmit without GSO now that we've disabled it.
                        // This is important for platforms (e.g. some Android kernels) that
                        // accept the setsockopt(UDP_SEGMENT) but then reject the actual segmented
                        // sendmsg with EIO. We send each segment as an individual datagram.
                        if e.raw_os_error() == Some(libc::EIO) && transmit.segment_size.is_some() {
                            let segment_size =
                                transmit.segment_size.unwrap_or(transmit.contents.len());
                            let fd = io.as_raw_fd();
                            for chunk in transmit.contents.chunks(segment_size) {
                                let no_gso_transmit = crate::Transmit {
                                ...
                                let mut seg_hdr: libc::msghdr = unsafe { mem::zeroed() };
                                let mut seg_iov: libc::iovec = unsafe { mem::zeroed() };
                                let mut seg_cmsgs = cmsg::Aligned([0u8; CMSG_LEN]);
                                let seg_dst = socket2::SockAddr::from(no_gso_transmit.destination);
                                prepare_msg(
                                    &no_gso_transmit,
                                    &seg_dst,
                                    &mut seg_hdr,
                                ...
                                loop {
                                    let n = unsafe { libc::sendmsg(fd, &seg_hdr, 0) };
                                    if n == -1 {
                                        let seg_e = io::Error::last_os_error();
                                        if seg_e.kind() == io::ErrorKind::Interrupted {
                                            continue;
                                        }
                                        if seg_e.raw_os_error() != Some(libc::EMSGSIZE) {
                                            return Err(seg_e);
                                        }
                                    }
                                    break;
